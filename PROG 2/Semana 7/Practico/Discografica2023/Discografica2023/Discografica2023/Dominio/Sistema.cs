...
namespace Dominio
{
    public class Sistema
    {
        ...
        {
            AltaCancion(new Cancion("Cancion 1", 5, 50));
            AltaCancion(new Cancion("Cancion 2", 3, 30));
        ...
        public List<Disco> DiscosPorValor(double monto)
        {
            List<Disco> retorno = new List<Disco>();

            foreach(Disco d in _discos)
            {
                if (d.ObtenerPrecio() >= monto) retorno.Add(d);
            }
            retorno.Sort();
            return retorno;
        }

        public List<Disco> DiscosEnCancion(int codigo)
        {
