...
namespace Dominio
{
    public class Sistema
    {
        ...
        private void PrecargarDiscos()
        {
            try
            {
                AltaDisco(new Disco("DISK001", ObtenerMusicoPorNombre("Cuarteto de Nos"), "Jueves", 2019, "DISK001.jpg"));
                AltaDisco(new Disco("DISK002", ObtenerMusicoPorNombre("Michael Jackson"), "Thriller", 1982, "DISK002.jpg") );
                AltaDisco(new Disco("DISK003", ObtenerMusicoPorNombre("Red Hot Chilli Peppers"), "Californication", 1999, "DISK003.jpg"));
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        {
            AltaUsuario(new Fan("fan@asd.com", "1234"));
            AltaUsuario(new Administrador("admin@asd.com", "1234", 1234));
        }
        ...
        public void CambiarTituloDisco(Disco d, string tituloNuevo)
        {
            //valido nulos
            //valido reglas de negocio EJ: no existan otros discos con el mismo nombre
            d.CambiarTitulo(tituloNuevo);
        }
        ...
        {
            if (string.IsNullOrEmpty(email)) throw new Exception("Datos vacios");
            Usuario u = null;
            int i = 0;
            while (u == null && i < _usuarios.Count)
            {
        ...
        public void HacerFavorito(Usuario usuario, Musico m)
        {
            if (usuario == null || m == null) throw new Exception("Datos nulos");
            Fan f = usuario as Fan;
            if (f == null) throw new Exception("Usuario solo puede ser fan");

            f.HacerFavorito(m);
        }

        public void DarLike(Disco d)
        {
            if (d == null) throw new Exception("Disco nulo");
            d.DarLike();
        }
    }
}
